...
public class FXMLModificaPromocionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        String[] tiposA = {"farmacia", "comercial", "cafeteria"};
    ...
    private void metedatos() {
        ...
        tipoField.setText(Utilidades.extractorNummeros(promocion.getTipo()) + "");
        tipoPromo = promocion.getCategoria();
        categoriaCombo.setValue(promocion.getCategoria());

        Empresa empresaEncontrada = EmpresaDAO.obtenerEmpresa(promocion.getIdEmpresa());
    ...
    private void cargarFotografiaServidor(File imagen) {
        try {
            ...
            Mensaje msj = PromocionDAO.subirFotografiaPromocion(imgBytes, promocion.getIdPromocion());
            if (!msj.getError()) {
    ...
    private void obtenerImagenServicio() {
        Promocion promocionFoto = PromocionDAO.obtenerFotografia(promocion.getIdPromocion());
        if (promocionFoto != null && promocionFoto.getImagenBase64() != null && !promocionFoto.getImagenBase64().isEmpty()) {
